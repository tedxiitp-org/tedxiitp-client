...
var Level = Base.extend({
	...
	reload: function() {
		...
		if(settings.lifes <= 0) {
			...
			window.parent.postMessage({ type: 'GAME_OVER', game: 'mario', score: settings.coins || 0 }, '*');
			...
			var goMusic = document.getElementById('gameover-music');
			if (goMusic) {
				goMusic.play().catch(function(){}); // Catch errors if file is missing
			}
	...
	load: function(level) {
		...
		this.world.parent().children('#levelNumber').text('Level: ' + (this.id + 1) + ' / ' + definedLevels.length);
	...
	nextLoad: function() {
		...
		var nextLevel = definedLevels[this.id + 1];
		if (!nextLevel) {
			// Game completed!
...
var Mario = Hero.extend({
	...
	setMarioState: function(state) {
		...
		if (state === mario_states.fire) {
			this.view.css('filter', 'hue-rotate(270deg) saturate(1.5)');
			$('#tc-fire').show();
		} else {
	...
	jump: function() {
		this.level.playSound(this.state === size_states.small ? 'jump-small' : 'jump-super');
...
var soundManager = {
	play: function(label) {
		var el = document.getElementById('sound-' + label);
		if (el) {
			el.currentTime = 0;
			el.play().catch(function(){});
		}
	},
	sideMusic: function(label) {
		var el = document.getElementById('music-' + label);
		if (el) {
			el.currentTime = 0;
			el.play().catch(function(){});
		}
...
$(document).ready(function() {
	$('#start-btn').on('click', function() {
		...
		if (!keys.isFullScreen && window.matchMedia("(pointer: coarse)").matches) {
